<template>
  ...
  <div class="messageBox" @click.self="cancel()" v-show="value">
    ...
    <!--QRCode弹窗-->
    <div class="alert QRCode" v-if="type==='QRCode'">
      <div class="title">
        <span>{{title}}</span>
        <!--<i class="iconfont icon-close" @click="cancel()"></i>-->
      </div>
      <div class="content">
        <slot name="content">
          <qrCodeEl :data="message"></qrCodeEl>
          <!--{{message}}-->
        </slot>
      </div>
      <div class="msgbox-btns">
        <slot name="confirmBtn">
          <i class="iconfont icon-close" @click="cancel()"></i>
          <!--<button class="msgbox-btn msgbox-confirm" @click="confirm()">确定</button>-->
        </slot>
      </div>
    </div>
    <!--confirm弹窗-->
    ...
    <!--背景-->
    <div @click="cancel()" class="v-modal" :style="{'opacity':isTransparent?0.001:0.5}"></div>
  </div>
</template>
<script>
  import qrCodeEl from '../qrcode/index';
  export default {
    components:{
      qrCodeEl
    },
